 ...
 * @version 1.0
 */
@NovaMapper(enumClass = ApproverType.class, enumValue = "COMPETE", mapperName = Constants.APPROVER_CALCULATOR)
public class CompeteApproverCalculator extends AbstractApproverCalculator implements ApproverCalcuator {
    ...
    public List<Approver> calculate(Nova nova) throws NovaException {
        ...
        logger.info("节点实例[{}], 节点[{}]竞办计算当前处理人开始", nova.getNodeInstId(), nova.getNodeId());
        ...
        if (Novas.isNullOrEmpty(nvApprovers)) {
            String errorMsg = Novas.formatMessage("节点实例[{}], 节点[{}]竞办当前配置定的审批人信息为空!", nova.getNodeInstId(), nova.getNodeId());
        ...
        logger.info("节点实例[{}], 节点[{}]竞办计算当前处理人结束, 审批人信息为:{}", nova.getNodeInstId(), nova.getNodeId());
        return apprvoers;
    }
